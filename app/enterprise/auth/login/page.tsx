...
/**
 * Enterprise Login Page — Stytch B2B Authentication
 *
 * Matches the individual user auth page design (split-panel layout).
 *
 * Supports:
 *   - Email/password (via Stytch)
 *   - Google OAuth (via Stytch)
 *   - Magic link (via Stytch)
 *   - Legacy password fallback (for users not yet migrated to Stytch)
 */
...
export default function EnterpriseLoginPage() {
  ...
  // Handle email submission — magic link or password
    ...
    if (showPasswordField) {
      ...
      try {
        ...
        if (response.ok && data.success) {
          ...
          const legacyToken = response.headers.get('X-Enterprise-Token');
          if (legacyToken) {
            localStorage.setItem('enterprise_token', legacyToken);
          }
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8">
        ...
        >
          ...
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-semibold text-gray-900">Sign in to VRIN</h1>
          ...
          <div className="mt-4 text-center">
            <button
              type="button"
              onClick={() => { setShowPasswordField(!showPasswordField); setPassword(''); setError(''); }}
          ...
          <div className="mt-6 text-center">
            <p className="text-sm text-gray-500">
              ...
              <Link href="/enterprise/auth/register" className="text-gray-700 hover:text-gray-900 font-semibold underline underline-offset-2">
                Create one
              </Link>
            </p>
          </div>
